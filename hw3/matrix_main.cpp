...
void StudentTest(){
	...
	Matrix m1 = Matrix(3,4,0);//makes a bunch of test Matricies
	Matrix m2 = Matrix(3,4,0);
	Matrix m3 = Matrix(3,4,1);
	Matrix m4 = Matrix(m3);
	Matrix m5 = Matrix();
	Matrix m6 = Matrix();
	Matrix m7 = Matrix(3,4,5);
	
	m5=m4;//tests assignment operator
	
	assert(m1==m2);//tests == operator
	assert(m1!=m3);//tests != operator
	assert(m3==m4);//tests copy constructor
	assert(m4==m5);//tests assignment operator
	assert(m3.num_rows()==3&&m3.num_cols()==4);//tests num_rows and num_cols
	assert(m6.num_rows()==0&&m6.num_cols()==0);//same as above with empty matrix
	
	m4.clear();//tests clear()
	m3.multiply_by_coefficient(10);//tests multiply_by_coefficient
	std::cout<<std::endl<<"M3:\n"<<m3<<"M4:\n"<<m4;//prints out tests
	
	m3.set(0,0,0);//tests set
	m3.set(1,1,1);
	m3.set(2,2,2);
	
	double v,f;//tests get
	assert(m3.get(2,2,v));//with good value
	assert(!m3.get(5,5,f));//with bad value
	std::cout<<std::endl<<"\nM3:\n"<<m3<<"(3,3): "<<v;//outputs the good value
	
	assert(m3.swap_row(1,2));//tests good swap_row
	assert(!m3.swap_row(1,10));//tests bad swap_row
	
	std::cout<<"\n\nSecond and third row swapped:\n"<<m3;
	m3.transpose();//tests transpose on 3x4
	std::cout<<"Transposed:\n"<<m3;
	assert(!m3.add(m5));//make sure you can't add different sized matricies
	m3.transpose();//tests transpose on 4x3
	std::cout<<"Transposed again:\n"<<m3;
	assert(m3.add(m5));//tests add
	std::cout<<"M3+M5:\n"<<m3;
	assert(m3.subtract(m7));//tests subtract
	std::cout<<"M3-M7:\n"<<m3;
	
	assert(m3.get_row(10)==NULL);//tests bad get_row
	assert(m3.get_col(10)==NULL);//tests bad get_col
	double* r1 = m3.get_row(0);//tests good get_row
	double* c3 = m3.get_col(2);//tests good get_col
	
	std::cout<<"First row: ";//prints out row
	for(int i=0; i<4; i++)
		std::cout<<" "<<r1[i];
	std::cout<<"\nThird column:\n";//prints out column
	for(int j=0; j<3; j++)
		std::cout<<" "<<c3[j]<<std::endl;
	delete [] r1; delete []  c3;//frees the row and column that were made
