...
int main() {
  ...
  for(int i=0; i<5; i++) {
		...
		std::cout << "Enter 2 integer coefficients to a quadratic function: x^2 + b*x + c = 0" << std::endl;
  ...
  for(int j=0; j<5; j++)  {
		...
		float root_1, root_2;
		bool success = find_roots(1,my_b[j],my_c[j], root_1,root_2);
	...
	if(root_1<root_2)	smaller_root[j]=root_1;
	else	smaller_root[j]=root_2;
	std::cout<< smaller_root[j] << std::endl;
    // Check our work...
    if (check_root(1,my_b[j],my_c[j], root_1) && check_root(1,my_b[j],my_c[j], root_2)) {
