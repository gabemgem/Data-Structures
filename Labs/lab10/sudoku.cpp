...
int Sudoku::Propagate(int i, int j) {
  ...
  int numChanged=0;
  int val=*(board[i][j].begin());

  for(int r=0; r<size; r++) {
    if(r==i) {continue;}
    set<int>::iterator iter=board[r][j].find(val);
    if(iter!=board[r][j].end()) {
      board[r][j].erase(iter);
      numChanged++;
    }
  }
  ...
  for(int c=0; c<size; c++) {
    if(c==j) {continue;}
    set<int>::iterator iter=board[i][c].find(val);
    if(iter!=board[i][c].end()) {
      board[i][c].erase(iter);
      numChanged++;
    }
  }
  ...
  if(i<quadrant_size) {
    ...
    if(j<quadrant_size) {
      ...
      for(int r=0; r<quadrant_size; r++) {
        for(int c=0; c<quadrant_size; c++) {
          if(r==i && c==j) {continue;}
          set<int>::iterator iter=board[r][c].find(val);
          if(iter!=board[r][c].end()) {
            board[r][c].erase(iter);
            numChanged++;
          }
        }
      }
      ...
      for(int r=0; r<quadrant_size; r++) {
        for(int c=j-(j%quadrant_size); c<(j-(j%quadrant_size))+quadrant_size; c++) {
          if(r==i && c==j) {continue;}
          set<int>::iterator iter=board[r][c].find(val);
          if(iter!=board[r][c].end()) {
            board[r][c].erase(iter);
    ...
    if(j<quadrant_size) {
      ...
      for(int r=i-(i%quadrant_size); r<(i-(i%quadrant_size))+quadrant_size; r++) {
        for(int c=0; c<quadrant_size; c++) {
          if(r==i && c==j) {continue;}
          set<int>::iterator iter=board[r][c].find(val);
          if(iter!=board[r][c].end()) {
