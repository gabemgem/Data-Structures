...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    if(!old_root)
      return NULL;
    TreeNode<T>* root = new TreeNode<T>(old_root->value);
    root->left=copy_tree(old_root->left);
    root->right=copy_tree(old_root->right);
  ...
  iterator find2(const T& key, TreeNode<T>* head) {
