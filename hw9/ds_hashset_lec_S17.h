...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
	...
	unsigned int hash_value = m_hash(key); //Gives which "bucket" key hashes to
    unsigned int index = hash_value % m_table.size(); //Need to map it to be in bounds (0...m_table.size()-1)
	hash_list_itr it = m_table[index].find(key);
	if(it == m_table[index].end()){
		m_table[index].push_front(key);
		m_size++;
		return make_pair(iterator(this,index,m_table[index].begin()),true);
	}
	else{
		return make_pair(iterator(this,index,it),false);
...
  // Erase the key 
  ...
  iterator begin() {
    // implemented in lecture or lab
	//Look for a spot with a non-empty list

	for(std::vector< std::list<KeyType> >::iterator it = m_table.begin() ; it!=m_table.end(); it++){
		if((*it).size()>0){
			return iterator(this,it-m_table.begin(), it->begin());
		}
	}
